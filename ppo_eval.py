...
if __name__ == '__main__':
	...
	critic.load_state_dict(ckpt_state_dict['ppo_critic_model_state_dict'])
	critic.eval()
	...
	with torch.no_grad():
		for batch_idx, data in tqdm(enumerate(target_iterator)):
			
			if batch_idx == 2:
				print ("WARNING! REMOVE THIS LINE BEFORE RUNNING ACTUAL EVALUATION!")
				print (f"THIS LINE EXITS AFTER 2 {SPLIT} batches, to ensure that this evaluation script works!")
				break
			...
			all_Vs.append(V)
			all_batch_rtgs.append(batch_rtgs)
			...
			all_surr1_cont.append(surr1_cont)
			all_surr2_cont.append(surr2_cont)
			all_surr1_disc.append(surr1_disc)
			all_surr2_disc.append(surr2_disc)
		...
		_surr1_cont = torch.cat(all_surr1_cont, dim=0)
		_surr2_cont = torch.cat(all_surr2_cont, dim=0)
		_surr1_disc = torch.cat(all_surr1_disc, dim=0)
		_surr2_disc = torch.cat(all_surr2_disc, dim=0)
	
		_V = torch.cat(all_Vs, dim=0)
		_batch_rtgs = torch.cat(all_batch_rtgs, dim=0)
	
		actor_loss = (-torch.min(_surr1_cont, _surr2_cont)).mean() + (-torch.min(_surr1_disc, _surr2_disc)).mean()
		critic_loss = nn.MSELoss()(_V, _batch_rtgs)
		
		avg_mae_log = sum(mae_log_vals)/len(mae_log_vals)
		print(f'{SPLIT}/avg_mae_log: ', avg_mae_log)
		avg_sc = sum(sc_vals)/len(sc_vals)
		print(f'{SPLIT}/avg_sc: ', avg_sc)
		print(f'{SPLIT}/actor_loss: ', actor_loss.item())
		print(f'{SPLIT}/critic_loss: ', critic_loss.item())